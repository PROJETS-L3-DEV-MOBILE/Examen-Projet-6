<div class="max-w-3xl bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    <h3 class="text-lg font-bold text-gray-900 mb-1">Effectuer un virement bancaire</h3>
    <p class="text-sm text-gray-500 mb-6">Transférer de l'argent d'un compte à un autre.</p>
    <form action="/transaction/virement" method="POST" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="num_compte_destination"
                    class="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-amber-500 focus:outline-none bg-white"
                    required>
                    <option value="">-- Sélectionner un compte --</option>
                    <?php foreach ($comptes ?? [] as $compte): ?>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Montant du virement *</label>
        ...
        <div class="flex justify-end pt-4 border-t border-gray-100">
            <button type="submit"
                class="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">Valider
                le virement</button>
        </div>
    </form>
</div>
